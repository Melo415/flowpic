...
if USE_SUBSET && num_samples > SUBSET_SIZE
    ...
    % 核心修复1：统一用列向量存储索引
    for c = 1:num_classes
        idx_c = find(labels == c);  % 返回列向量（n×1）
        if ~isempty(idx_c)
            % 每个类别取固定数量，不足则取全部
            num_take = min(samples_per_class, length(idx_c));
            % 随机采样（保持列向量）
            take_idx = idx_c(randperm(length(idx_c), num_take));
            subset_idx = [subset_idx; take_idx];  % 垂直拼接（列向量）
        end
    end
    
    % 核心修复2：补足到SUBSET_SIZE（统一维度）
    current_subset_size = length(subset_idx);
    if current_subset_size < SUBSET_SIZE
        % 需要补充的样本数
        need_more = SUBSET_SIZE - current_subset_size;
        % 生成所有样本的索引（列向量）
        all_idx = (1:num_samples)';
        % 排除已选的索引
        remaining_idx = all_idx(~ismember(all_idx, subset_idx));
        % 随机选需要补充的数量（保持列向量）
        if length(remaining_idx) >= need_more
            add_idx = remaining_idx(randperm(length(remaining_idx), need_more));
        else
            add_idx = remaining_idx;  % 不足则取全部
        end
        % 垂直拼接（列向量+列向量）
        subset_idx = [subset_idx; add_idx];
    end
    
    % 去重（确保无重复索引）
    subset_idx = unique(subset_idx);
    % 确保最终子集大小不超过SUBSET_SIZE
    if length(subset_idx) > SUBSET_SIZE
        subset_idx = subset_idx(1:SUBSET_SIZE);
    end
